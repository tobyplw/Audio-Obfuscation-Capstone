...
def talk(udp_socket, record_stream, destination_ip, destination_port):
    ssrc = 5678 
    payload_type = 0  
    #voc = Vocoder(create_random_seed = False, rate = RATE, chunk = CHUNK_SIZE, distortion=0.5)
    ...
    try:
        while True:
            ...
            in_data = record_stream.read(CHUNK_SIZE)
            #in_data = voc.transform(in_data)
            #in_data = voc.float2pcm(in_data)
            #packed_data = struct.pack("%di"%len(in_data), *in_data)
            packet = rtp_header + in_data
...
def listen(udp_socket, listen_stream):
    ...
    previous_time = 0
    prev_play_time = 0
    print("Recieving audio from " + recieving_ip)
    try: 
        while True:
            try:
...
# destination_ip = '74.135.7.54' # Toby Public ipv4
